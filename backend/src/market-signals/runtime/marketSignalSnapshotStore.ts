...
export type MarketSignalSnapshotFreshness = {
  ready: boolean
...
export type MarketSignalSnapshotState = {
  snapshot: MarketSignalSnapshot
  freshness: MarketSignalSnapshotFreshness
}

type SetMarketSignalSnapshotOptions = {
...
function buildInitialSnapshot(): MarketSignalSnapshot {
  return {
    status: 'warming',
...
export class MarketSignalSnapshotStore {
  private snapshot: MarketSignalSnapshot = buildInitialSnapshot()
  ...
  setSnapshot(snapshot: MarketSignalSnapshot, options: SetMarketSignalSnapshotOptions = {}) {
  ...
  getSnapshot(now = new Date()): MarketSignalSnapshotState {
...
export function createMarketSignalSnapshotStore(refreshIntervalMs?: number) {
  return new MarketSignalSnapshotStore(refreshIntervalMs)
}

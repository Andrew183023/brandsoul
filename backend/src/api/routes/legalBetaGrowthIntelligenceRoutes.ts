...
import { buildGrowthIntelligenceResponse } from '../../modules/legalGrowth/growthIntelligenceController.js'
  ...
  if (!isOwnedByAuth(entity, auth.userId, auth.tenantId)) {
    await reply.status(403).send({
      ...
      error: {
        ...
        message: 'Only the office owner can access this growth intelligence surface.',
      },
    })
    return null
  }
...
const privateReadRateLimit = createRateLimit({
  namespace: 'legal-beta-office-growth-intelligence-read',
  max: 120,
  windowMs: 60_000,
  key: 'user',
})

export async function registerLegalBetaGrowthIntelligenceRoutes(app: FastifyInstance) {
  app.get<{ Params: { id: string } }>(
    '/escritorios/:id/growth-intelligence',
    ...
    async (request, reply) => {
      ...
      const [cases, professionals] = await Promise.all([
        getCaseRepository(app).listCasesByEntity(owned.tenantId, owned.officeId),
        getCaseService(app).listOfficeProfessionals(owned.tenantId, owned.officeId),
      ])

      return buildGrowthIntelligenceResponse({

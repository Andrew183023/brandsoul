...
async function loadAuthOwnershipMaps(db: BackendDatabase, rows: EntityProfileRow[]) {
  ...
  return {
    ...
    legacyTenants,
    legacyMemberships,
    nativeUserMap: new Map(nativeUsers.map((row) => [Number(row.id), row])),
    nativeTenantMap: new Map(nativeTenants.map((row) => [Number(row.id), row])),
    nativeMembershipMap: new Map(nativeMemberships.map((row) => [`${Number(row.user_id)}:${Number(row.tenant_id)}`, row])),

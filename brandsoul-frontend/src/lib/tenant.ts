import { fetchCurrentTenant } from './auth'
import { getAuthToken, loadSession, saveSession, type AuthTenant } from './session'
...
export function buildPublicBrandUrl(slug?: string | null) {
  return slug ? `${window.location.origin}/brands/${slug}` : `${window.location.origin}/`
}

...
import { buildBaseRendererOutput, buildPlanMaterializedRendererOutput, hasRealShapeSource, hasVisualBodyPlan, resolveVisualBodyAnchor, resolveVisualBodyCore, resolveVisualBodyFraming } from '../shared/rendererShared'
...
function buildRoboFallbackOutput(input: PersonaRenderInput, baseOutput: PersonaRenderOutput): PersonaRenderOutput {
  ...
  const gridSizeBase =
    signature?.dominantAxis === 'vertical'
      ? 16
      : signature?.dominantAxis === 'horizontal'
        ? 14
        : signature?.type === 'orbital'
          ? 10
          : 12
  const gridSize = artDirection.massDistribution.includes('grid')
    ? Math.max(8, gridSizeBase - dnaModulators.shapeRigidityBias * 8)
    : Math.max(10, gridSizeBase - 2 - dnaModulators.shapeRigidityBias * 4)
  ...
  const emitterConfig = baseOutput.particles.emitterConfig
    ? {
        ...
        direction:
          behavior.birth === 'scan-and-assemble'
            ? {
                angle: signature?.dominantAxis === 'vertical' ? 90 : 0,
                spread: signature?.type === 'linear' ? 64 : 96 - (signature?.symmetry ?? 0.5) * 20 + dnaModulators.particleDensityBias * 50,
              }
            : baseOutput.particles.emitterConfig.direction,
      }
    : undefined
...
export function roboRenderer(input: PersonaRenderInput): PersonaRenderOutput {
  ...
  if (hasVisualBodyPlan(input)) {
    return buildPlanMaterializedRendererOutput({
      baseOutput,
      emitterOrigin: planScanAnchor,
      emitterDirection: behavior.birth === 'scan-and-assemble' ? { angle: 0, spread: 96 } : baseOutput.particles.emitterConfig?.direction,
      innerPath:
  ...
  return buildRoboFallbackOutput(input, baseOutput)
}
